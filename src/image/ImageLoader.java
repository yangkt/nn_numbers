...
public class ImageLoader {
	...
	public class DigitImage {
		...
		public DigitImage(int label, byte[] image) {
			...
			pixels = new double[image.length];

			// convert it to 0 and 1 pixel map
			for (int i = 0; i < image.length; ++i) {
				pixels[i] = (image[i] < 0 ? 1 : 0);
			}
		}
	...
	public ImageLoader(String labelFileName, String imageFileName) {
	...
	public List<DigitImage> loadDigitImages() throws IOException {
		...
		// this.getClass().getResourceAsStream(labelFileName);
		// InputStream imageInputStream =
		// this.getClass().getResourceAsStream(imageFileName);
		...
		for (int i = 0; i < numberOfLabels; i++) {
			// for(int i = 0; i < 10000; i++) { // just get the first 1000 images
			...
			DigitImage theImage = new DigitImage(label, imageData);
			// theImage.print();
